...
func BuildSystemInstruction(in SystemInstructionInput) string {
	...
	sb.WriteString(fmt.Sprintf("- 章节文件名模板：%s；编号支持多位数字，千章作品推荐使用 ch{NNNN}-{title}.md；若大纲、进度或前文路径显示当前章节属于某一卷，模板应用在对应分卷目录下，例如 chapters/第一卷/ch0001-废材开局.md、chapters/第二卷/ch0101-重返王都.md。\n", normalizedChapterFilenameFormat(in.ChapterFilenameFormat)))
...
func BuildLoreAgentSystemInstruction(in LoreAgentSystemInstructionInput) string {
	...
	sb.WriteString("- 可以使用文件读写工具读取工作区文件；初始化流程最终只允许写入 CREATOR.md，不写 脑暴.md、chapters/、setting/outline.md、setting/progress.md、setting/character-states.md。\n")
	...
	sb.WriteString("- 初始化 v1 不写 脑暴.md，不生成大纲，不创建互动故事；互动开局先作为资料库和 CREATOR.md 中的设定沉淀。\n\n")
	sb.WriteString("## 输出方式\n")
...
- %s/chapters/ — 章节正文（按配置的章节文件名模板命名；可按大纲分卷创建子目录，例如 chapters/第一卷/ch0001-废材开局.md）
...
### 生成大纲时
1. 先 read_file 脑暴.md 和 CREATOR.md，与作者一起讨论补全顶层定调和创作者指令；脑暴.md 负责“这本书是什么”，CREATOR.md 负责“这本书长期怎么写、哪些规则必须一直遵守”
2. 基于脑暴.md 模板确认题材、卖点、读者、整体风格、金手指、故事尺度、剧情走向、参考作品等；字段仍为模板占位或留空时，不要直接进入下一步，先引导作者完善
3. 基于 CREATOR.md 模板确认基本创作内容，包括每章字数/篇幅目标、禁止内容、写作风格、叙事视角、对话风格和其他全局要求；字段仍为模板占位、示例内容或留空时，不要直接进入下一步，先引导作者逐项确认
4. 作者明确确认后，先分别 write_file 更新 脑暴.md 和 CREATOR.md，确保两份模板都沉淀为本书的定稿规则，再生成 setting/outline.md
...
7. 大纲生成后，脑暴.md 不再修改，也不再作为后续创作上下文参考；CREATOR.md 继续作为每轮最高优先级创作者指令生效，可在作者后续明确要求调整全局创作规则时更新
...
const emptyStateHint = "这是一个新的作品，尚未生成大纲和资料库。请先打开作品根目录下的 `脑暴.md` 和 `CREATOR.md`，基于两份模板与作者确认顶层定调和基本创作规则：脑暴.md 确认题材、核心卖点、目标读者、整体风格、剧情走向等；CREATOR.md 确认每章字数、禁止内容、写作风格、叙事视角、对话风格和其他全局要求。待作者明确确认后，先写回 `脑暴.md` 和 `CREATOR.md`，再生成 setting/outline.md、setting/progress.md、setting/character-states.md，并把角色、世界观、地点、势力、规则和物品等长期设定整理到资料库。在此之前不要直接编造大纲或角色。"
